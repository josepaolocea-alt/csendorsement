(function(){
  ...
  function start(){
    ...
    range.addEventListener('input',function(){scroll.scrollLeft=Number(range.value)||0;sync();});
    previous.addEventListener('click',function(){move(-Math.max(260,scroll.clientWidth*.72));});
    next.addEventListener('click',function(){move(Math.max(260,scroll.clientWidth*.72));});
    rail.addEventListener('wheel',function(event){
      ...
      if(!amount)return;
      event.preventDefault();scroll.scrollLeft+=amount;sync();
    },{passive:false});
